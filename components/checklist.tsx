...
export const checklistSections=[
 ...
 {title:'防護',subtitle:'減少可以被比對的線索',Icon:Link2,items:['運動紀錄：限制可見度，隱藏起訖點、地圖與開始時間。','發布方式：避免即時分享固定路線，回頭檢查舊紀錄。','公開文件：另製公開版，移除識別欄位、QR Code 與中繼資料。','帳號分流：依用途區分名稱、頭像與簡介，檢查跨平台連結。']},
...
export function Checklist({onBack}:{onBack:()=>void}) {
 ...
 <article className="checklist-paper"><header className="checklist-heading"><div><div className="paper-kicker">FOOTPRINT / PROTECT</div><h1>辨識・防護・回查</h1><p>我的數位足跡防護檢核卡</p></div><ScanLine size={40}/></header>
 <p className="checklist-intro">先辨識，再調整防護方式，最後從陌生人視角回查。勾選代表已完成檢查；若仍看得到資訊，就回到「防護」再調整。</p>
 ...
 <aside className="paper-reminder"><strong>再請另一位信任的人交叉檢查。</strong><p>只檢查自己的公開內容，不蒐集或追蹤他人的資料。這張卡用於降低可被串聯的機會，無法保證完全安全。</p></aside><footer className="paper-footer">足跡防護｜跨平台數位足跡風險防護 <span>A4 檢核卡 · 1 / 1</span></footer></article>
