...
function Profile({name='shenbike24',different=false}:{name?:string;different?:boolean}) {
...
function ActivityCard({safe=false,compact=false}:{safe?:boolean;compact?:boolean}) {
 return <article className="evidence-card"><CardHead kind="activity" label="步頻 · 運動紀錄" protectedView={safe}/><Profile name={safe?'trail_notes':'shenbike24'}/><h3>{safe?'今日的運動小記':'晨間河岸騎乘'}</h3>
...
function SocialCard({safe=false}:{safe?:boolean}) {
 return <article className="evidence-card"><CardHead kind="social" label="片刻 · 社群簡介" protectedView={safe}/><Profile name={safe?'little_weekend':'shenbike24'} different={safe}/><div className="social-copy"><h3>{safe?'收藏日常的小片刻':'騎車，也分享學習筆記'}</h3><p>{safe?'沒有姓名、學校、固定行程，也沒有其他平台連結。':'運動紀錄與學習文件都使用 @shenbike24。'}</p><span className={safe?'safe-chip':'account-chip'}>{safe?'名稱、頭像與簡介依用途分流':'與其他平台使用相同名稱與頭像'}</span></div></article>;
}
