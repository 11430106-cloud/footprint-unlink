...
const stages=[{name:'辨識',label:'發現公開線索',Icon:Fingerprint},{name:'防護',label:'選擇防護方式',Icon:Link2},{name:'回查',label:'換個視角檢查',Icon:ScanLine}];
...
export default function Home() {
 ...
 <header className="site-header"><div className="brand"><span className="brand-mark"><ScanLine size={25}/></span><span>足跡防護<small>FOOTPRINT / PROTECT</small></span></div><Button variant="ghost" className="help-button" onClick={()=>setOverlay(overlay==='tutorial'?null:'tutorial')}><CircleHelp size={18}/><span>使用教學</span></Button></header>
 ...
 {state.view==='intro'&&<main className="intro-main"><div className="case-label"><span className="signal"/> 任務簡報 <span>CASE 001</span></div><h1>看見線索之間的連結。</h1><p className="lead">你將觀察虛構角色「沈有梭」留下的公開資料，<br className="desktop-break"/>依序完成辨識、防護與回查，再把方法帶回自己。</p><div className="clue-grid">{[{Icon:Fingerprint,title:'身分',copy:'姓名、學校、信箱、照片、學號'},{Icon:MapPin,title:'位置',copy:'運動路線、住家附近、固定起訖點'},{Icon:Clock3,title:'時間',copy:'上課、通勤、運動的固定時段'},{Icon:Link2,title:'關聯',copy:'相同帳號、頭像、簡介或朋友'}].map(({Icon,title,copy})=><div className="clue-item" key={title}><Icon/><h2>{title}</h2><p>{copy}</p></div>)}</div><div className="instruction-note"><Lightbulb size={23}/><div><strong>先看資料，再下判斷。</strong><p>每題標示單選或複選；確認答案後會顯示解說。複選需完整答對才得分，沒有時間限制。</p></div></div><div className="button-row"><Button variant="ghost" className="text-button" onClick={()=>act({type:'reset'})}><ArrowLeft/>返回首頁</Button><Button className="primary-button" onClick={()=>act({type:'start'})}>開始辨識 <ArrowRight/></Button></div></main>}
